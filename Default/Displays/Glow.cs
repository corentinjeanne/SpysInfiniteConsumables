...
public sealed class GlowConfig {
    ...
    [DefaultValue(1.2f), Range(0f, 3f), Increment(0.1f)] public float Size = 1.2f;
}
...
public sealed class Glow : Display, IConfigProvider<GlowConfig> {
    ...
    public static void DisplayGlow(SpriteBatch spriteBatch, Item item, Vector2 position, Rectangle frame, Vector2 origin, float scale, Color color) {
        ...
        if (!Instance.Config.FancyGlow) {
            float scl = 1 + 8 * distance / frame.Width * Instance.Config.Size;
            spriteBatch.Draw(texture, position, frame, color, 0, origin, scale * scl, 0, 0f);
            return;
        }

        for (float f = 0f; f < 1f; f += 1 / 3f) spriteBatch.Draw(texture, position + new Vector2(0f, 1.5f + 1.5f * distance * Instance.Config.Size).RotatedBy((f * 2 + progress) * Math.PI), frame, color, 0, origin, scale, 0, 0f);
        color *= 0.67f;
        for (float f = 0f; f < 1f; f += 1 / 4f) spriteBatch.Draw(texture, position + new Vector2(0f, 4f * distance * Instance.Config.Size).RotatedBy((f + progress) * -2 * Math.PI), frame, color, 0, origin, scale, 0, 0f);
    }
