...
public sealed class Defaults : Preset {
    ...
    public override bool MeetsCriterias(GroupConfig config) {
        foreach ((InfinityDefinition def, bool enable) in config.Infinities.Items<InfinityDefinition, bool>()) {
            if (!enable == InfinityManager.GetInfinity(def.Mod, def.Name)?.DefaultEnabled()) return false;
        }
    ...
    public override void ApplyCriterias(GroupConfig config) {
        for(int i = 0; i < config.Infinities.Count; i++) {
            ...
            if(!def.IsUnloaded) config.Infinities[i] = InfinityManager.GetInfinity(def.Mod, def.Name)!.DefaultEnabled();
        }
...
public sealed class Classic : Preset {
    ...
    public override void ApplyCriterias(GroupConfig config) {
        ...
        foreach(IInfinity infinity in Order) config.Infinities.Add(new InfinityDefinition(infinity), infinity.DefaultEnabled());
        foreach(IInfinity infinity in Infinities.Items.Instance.Infinities) config.Infinities.TryAdd(new InfinityDefinition(infinity), infinity.DefaultEnabled());
        config.UsedInfinities = 1;
    }
}
